...
报告输出到 `statistics/` 目录，默认生成**两个文件**：

| 文件 | 说明 | 体积 |
|------|------|------|
| `告警统计分析报告_<ts>.html` | **完整版**：摘要卡片、四维筛选、动态图表、TOP 告警、明细表格、自定义导出、CSV 导出 | ~120 MB |
| `告警统计分析报告_<ts>_lite.html` | **精简版**：完整版去掉自定义导出与原始数据 JSON，保留全部筛选/图表/明细/CSV 导出 | ~200 KB |

> 精简版通过 `_strip_lite()` 按元素精确删除生成，确保多维度筛选功能不受影响。

### 生成后报告交付

**分析完成后，Agent 应将报告发送给用户，而非仅打印路径。**

1. **优先发送精简版** — 体积小（~200KB），含全部筛选/图表/CSV 导出
2. **完整版按需发送** — 通常超过 100MB，仅在用户明确要求时提供
3. **发送方式**：通过消息平台附带文件，或使用 `MEDIA:<文件路径>` 标记
4. **无法发送时**：明确告知报告路径，引导用户自行打开
